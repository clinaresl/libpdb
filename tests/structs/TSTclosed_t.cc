...
//
// Unit tests for testing closed lists
//
...
TEST_F (ClosedFixture, Empty) {
    ...
    for (auto i = 0 ; i < NB_TESTS ; i++) {
        ...
        // create an empty closed list (of pancakes) and verify its size and
        // capacity are correct. closed lists, when being created have to know the
        // number of items to host. An arbitrary value is given here.
        pdb::closed_t<npancake_t> closed (MAX_VALUES);

        ASSERT_EQ (closed.capacity (), MAX_VALUES);
...
// ----------------------------------------------------------------------------
TEST_F (ClosedFixture, InsertNPancake) {

    for (auto i = 0 ; i < NB_TESTS/10 ; i++) {

        // create an empty closed list (of n-pancakes)
        int capacity = 1 + rand () % MAX_VALUES;
        pdb::closed_t<npancake_t> closed (capacity);
...
/*         // create an empty closed list (of n-pancakes) */
/*         pdb::closed_t<pdb::node_t<npancake_t>> closed; */
...
/*         // vector. */
/*         vector<pdb::node_t<npancake_t>> nodes; */
/*         for (auto value : values) { */
/*             auto ptr = closed.insert (value); */
...
/*             closed.insert (const_cast<pdb::backnode_t<npancake_t>&>(value)); */
/*         } */
...
/*         // create an empty closed list (of n-pancakes) */
/*         pdb::closed_t<pdb::backnode_t<npancake_t>> closed; */
...
/*             // list */
/*             pdb::backpointer_t bp{backpointer, 0}; */
/*             path[i] += bp; */
...
/*         std::deque<pdb::backnode_t<npancake_t>> reconstructed; */
